...
// case 2 : 삭제할 노드가 하나의 자식 노드인 경우
// case 3 : 삭제할 노드가 두 개의 자식 노드인 경우
// case 4~6 : 삭제할 노드가 루트 노드인 경우.
...
BTreeNode *BSTRemove(BTreeNode **pRoot, BSTData target)
{
	...
	//cNode = BSTSearch(*pRoot, target); 을 사용하면 cNode의 부모 노드를 pNode가 가리키게 할 수 없다.
	...
	// 삭제할 노드의 오른쪽 서브 트리에서 가장 작은 값을 지니는 노드를 찾아 대체.
	else
	{
		BTreeNode *mNode = GetRightSubTree(dNode);    // mininum node, 삭제할 값의 오른쪽 노드
